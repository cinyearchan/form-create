...
export default defineComponent({
  ...
  methods: {
    ...
    makeItem(index, children) {
      return (
        <div class="_fc-files" key={"3" + index}>
          {children}
        </div>
      )
    },
    ...
    makeIcons(val, index) {
      if (this.handleIcon !== false || this.allowRemove === true) {
        ...
        return (
          <div class="_fc-upload-cover" key={4}>
            {icons}
          </div>
        )
      }
    ...
    makeHandleIcon(val, index) {
      const Type = resolveComponent(
        this.handleIcon === true || this.handleIcon === undefined
          ? "icon-view"
          : this.handleIcon
      )
      return (
        <ElIcon onClick={() => this.handleClick(val)} key={"5" + index}>
          <Type />
        </ElIcon>
      )
    },
    ...
    makeRemoveIcon(val, index) {
      return (
        <ElIcon onClick={() => this.handleRemove(val)} key={"6" + index}>
          <IconDelete />
        </ElIcon>
      )
    },
    ...
    makeFiles() {
      return this.makeGroup(
        this.fileList.map((src, index) => {
          return this.makeItem(index, [
            <ElIcon onClick={() => this.handleClick(src)}>
              <IconDocument />
            </ElIcon>,
    ...
    makeBtn() {
      ...
      return (
        <div class="_fc-upload-btn" onClick={() => this.showModel()} key={7}>
          <ElIcon>
            <Type />
          </ElIcon>
        </div>
      ...
      return (
        <div>
          {closeBtn ? (
            <ElButton
              onClick={() =>
                this.onCancel() !== false && (this.frameVisible = false)
              }
            >
              {closeBtnText}
            </ElButton>
