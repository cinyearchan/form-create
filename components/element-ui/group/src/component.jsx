...
export default defineComponent({
  ...
  watch: {
    ...
    expand(n) {
      const d = n - this.modelValue.length
      if (d > 0) {
        this.expandRule(d)
      }
    },
  ...
  methods: {
    ...
    setValue(key, value) {
      ...
      if (
        this.cacheValue[key] === JSON.stringify(field ? value[field] : value)
      ) {
        return
      }
    ...
    addIcon(key) {
      return (
        <ElIcon
          key={`a${key}`}
          style={`font-size:${this.fontSize}px;cursor:${
            this.disabled ? "not-allowed;color:#c9cdd4" : "pointer"
          };`}
          onClick={this.add}
        >
          <IconCirclePlus />
  ...
  render() {
    ...
    return keys.length === 0 ? (
      this.$slots.default ? (
        ...
        <ElIcon
          key={1}
          style={`font-size:${this.fontSize}px;vertical-align:middle;color:${
            this.disabled
