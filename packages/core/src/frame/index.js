...
export default function FormCreateFactory(config) {
  ...
  function register() {
    ...
    if (data.id && data.prop)
      providers[data.id] = { ...data.prop, name: data.id }
  }
  ...
  function create(rules, option) {
    const app = createFormApp(rules, option || {})
    useApps.forEach(v => {
      invoke(() => v(create, app))
    })
  ...
  extend(FormCreate.prototype, {
    ...
    initOptions(options) {
      ...
      if (this.isSub()) {
        this.options.value = this.mergeOptions(
          this.options.value,
          this.vm.setupState.parent.setupState.fapi.config || {},
          true
        )
      }
    ...
    updated() {
      this.$handle.bindNextTick(() =>
        this.bus.$emit("next-tick", this.$handle.api)
      )
    }
  })
