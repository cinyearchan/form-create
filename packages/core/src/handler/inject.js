...
export default function useInject(Handler) {
  extend(Handler.prototype, {
    ...
    parseEvent(rule, fn, inject, deep) {
      if (
        is.Function(fn) &&
        ((inject !== false && !is.Undef(inject)) || fn.__inject)
      ) {
    ...
    parseEmit(ctx) {
      ...
      if (is.trueArray(emit)) {
        const emitKey = emitPrefix || field || name
        if (emitKey) {
          emit.forEach(eventName => {
            ...
            if (!eventInject && inject === false) {
              ...
              const _inject = eventInject || inject || this.options.injectEvent
              event[eventName] = is.Undef(_inject)
                ? fn
                : this.inject(rule, fn, _inject)
            }
          })
        }
      }
