...
export default function useContext(Handler) {
  extend(Handler.prototype, {
    ...
    watchCtx(ctx) {
      const all = attrs()
      all
        .filter(k => k[0] !== "_" && k[0] !== "$" && noneKey.indexOf(k) === -1)
        .forEach(key => {
          ...
          ctx.watch.push(
            watch(
              flag
                ? () =>
                    is.Function(ref.value) ? ref.value : [...(ref.value || [])]
                : () => ref.value,
              (_, o) => {
                let n = ref.value
                ...
                } else if (key === "emit") this.parseEmit(ctx)
            ...
            watch(
              () => {
                ...
                if (is.Function(computed)) {
                  ...
                  fn = () =>
                    new Function("_", `with(this){ return ${computed} }`).call(
                      this.api.form,
                      _
                    )
                }
