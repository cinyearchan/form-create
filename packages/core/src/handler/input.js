...
export default function useInput(Handler) {
  extend(Handler.prototype, {
    ...
    onInput(ctx, value) {
      ...
      if (
        ctx.input &&
        (this.isQuote(ctx, (val = ctx.parser.toValue(value, ctx))) ||
    ...
    isChange(ctx, value) {
      return (
        JSON.stringify(this.getFormData(ctx), strFn) !==
        JSON.stringify(value, strFn)
      )
    },
    isQuote(ctx, value) {
      return (
        (is.Object(value) || Array.isArray(value)) && value === ctx.rule.value
      )
    },
