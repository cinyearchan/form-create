  ...
  extend(Handler.prototype, {
    ...
    _loadRule(rules, parent) {
      const preIndex = i => {
        ...
        const index = this.sort.indexOf(pre.__fc__.id)
      ...
      rules.map((_rule, index) => {
        ...
        const isRepeat = () => {
          return !!(
            rule.field &&
            this.fieldCtx[rule.field] &&
            this.fieldCtx[rule.field][0] !== _rule.__fc__
          )
        }
    ...
    useCtrl(ctx) {
      ...
      this.deferSyncValue(() => {
        validate
          .reverse()
          .forEach(
            ({ isHidden, valid, rule, prepend, append, child, ctrl }) => {
              if (isHidden) {
                valid
                  ? ctx.ctrlRule.push({
                      __ctrl: true,
