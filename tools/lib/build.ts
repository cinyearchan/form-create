...
import {
  blue,
  getFolderNames,
  getSingleComponentPaths,
  getSinglePackagePaths,
  // red,
  targets as getAllTargetsPath,
  yellow
} from "./utils"
...
async function buildAllComponents(paths: (string | undefined)[] = []) {
  if (!paths) {
    paths = []
  }
  const componentsPathsObject = Object.create(null)
  paths.forEach((comPath: string | undefined) => {
    if (comPath) {
      ...
      if (!componentsPathsObject[name]) {
        ...
        componentsPathsObject[name] = [
          ...componentsPathsObject[name],
          ...componentsPaths[name]
        ]
      }
    }
  })
...
async function createBuildTask(args: any) {
  ...
  if (isAll) {
    ...
    tips =
      "    build task time" + " " + dayjs().format("YYYY-MM-DD HH:mm:ss") + "\n"
  }
  yellow(tips, true)
  blue("    ---------------------- start ----------------------\n", true)
  /// not all components
  if (!isComponentsAll && args?.components?.length) {
    ...
    if (libs.length === 1) {
      ...
      if (lib.includes("*") || !lib.includes("/")) {
        ...
        const componentsPaths = getSingleComponentPaths(
          "components",
          name,
          cname
        )
      ...
      const allPaths = compsPaths.filter(
        cp => cp.includes("*") || !cp.includes("/")
      )
      const fixPaths = compsPaths.filter(
        cp => !cp.includes("*") && cp.includes("/")
      )
      let _all_ = [...fixPaths]
      if (allPaths.length) {
  ...
  if (isAll) {
    ...
    if (
      !Object.keys(componentsAllPaths).length &&
      !Object.keys(packagesAllPaths).length
    ) {
      yellow(
