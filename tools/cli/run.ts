...
program.on("--help", () => {
  ...
  log(
    ...
    chalk.cyan.bold("tools build tools with Node 👍 ~")
  )
  log()
})
...
/// tools 版本
program.version(
  `@form-create/tools v${require(pkgUrl).version}`,
  "-v, --version",
  "tools versions"
)
...
program
  .command("build [flag]")
  .description(
    "build components && packages || build components || build packages"
  )
  ...
  .option(
    "-p, --packages <ui-package...>",
    "Build @form-create/<ui-package> package or packages array"
  ) // 打单独的包
